...
2. Explain in detail the similaraities and differences between Gaussian
...
completed the course eval by the final due date on Thursday!
...
- Describe the general segmentation / changepoint detection
  ...
  are the outputs?
- concept of loss function which always decreases as the model size
  increases for train set and U shape for validation set.
