...
2. Now use mclust to fit a model with 5 clusters to the scaled data
   ...
   data. Use the mclust::dens(logarithm=FALSE) function to compute the mixture density
   on a grid of points from the min to the max of your random data,
   and plot with geom_line. Do the two models look similar?
