...
2. In viz.aligned the scatterplot displays fertility rate and life
   ...
   series. Hint: add another geom_point and use both horizontal and
   vertical panels in facet_grid(row_var ~ col_var).
3. Create a multi-panel data visualization that shows each year of the
   WorldBank data set in a separate panel.
