...
- Run HMM for nstates=1 to 10, and compute the number of segments in
  ...
  number of segments, penalty="Manual", method="SegNeigh").
...
- Compute the negative log likelihood using
  ...
  likelihood). Plot the negative log likelihood as a function of the
  number of segments, different algorithms in different colors (e.g.,
  red=HMM, black=DP). Is the curve lower for DP as expected?
